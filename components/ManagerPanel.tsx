...
import { User, PermitRequest, PermitStatus } from '../types';
...
const ManagerPanel: React.FC<ManagerPanelProps> = ({ user, requests, onApprove, onReject }) => {
  ...
  const applyFilters = useCallback((forceAll: boolean = false) => {
    const results = requests.filter(req => {
      // Regla Mandatoria: Solo ve su sector
      const belongsToSector = req.sector === user.sector;
  ...
  const handleFilterClick = () => {
    ...
    if (isFilterEmpty) {
      confirmAction(
        ...
        'No has aplicado filtros. ¿Estás seguro de que quieres visualizar todas las solicitudes históricas de tu sector?',
        'info',
        () => applyFilters(true)
      );
    } else {
  ...
  const showHistoryDetails = (req: PermitRequest) => {
    const history = [
      { label: 'Solicitud Creada', value: `${req.date} ${req.time}hs` },
      { label: 'Aprobación Jefe', value: req.bossApprovalDate ? new Date(req.bossApprovalDate).toLocaleString() : 'Pendiente' },
      { label: 'Validación RRHH', value: req.hrApprovalDate ? new Date(req.hrApprovalDate).toLocaleString() : 'Pendiente' },
    ...
    alert(`Línea de Tiempo - Permiso ${req.id}\n\n${message}`);
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tight">Auditoría de Sector: {user.sector}</h2>
          <p className="text-slate-500 font-medium">Control total e historial de movimientos de su equipo</p>
        </div>
      </div>
        ...
        {!hasSearched ? (
          <div className="py-24 text-center px-6">
            <div className="bg-slate-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4">
              <i className="fas fa-search text-indigo-300 text-3xl"></i>
            </div>
            <h3 className="text-xl font-bold text-slate-800 mb-2">Consulta de Auditoría</h3>
            <p className="text-slate-400 font-medium max-w-xs mx-auto">Use los filtros superiores para consultar el historial de permisos de su sector.</p>
          </div>
          ...
          <div className="py-24 text-center px-6">
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-2">Sin registros encontrados</h3>
            <p className="text-slate-400 font-medium">No hay registros históricos que coincidan con los criterios en {user.sector}.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  ...
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Motivo / Tipo</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Estado Actual</th>
                  ...
                  <tr key={req.id} className="hover:bg-indigo-50/10 transition-colors group">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-slate-400 font-medium">{req.time} hs</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex flex-col max-w-[200px]">
                         <span className="text-[10px] font-black text-indigo-500 uppercase mb-0.5">{req.type}</span>
                         <span className="text-xs text-slate-500 italic truncate" title={req.motive}>"{req.motive}"</span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-5">
                      <div className="flex justify-center gap-2">
                        {req.status === PermitStatus.PENDING_BOSS && (
                          <>
                            <button 
                              onClick={() => confirmAction(
                                'Autorizar Salida', 
                                `¿Autorizas el permiso para ${req.employeeName}? Pasará a revisión de RRHH.`, 
                                'success', 
                                () => onApprove(req.id)
                              )}
                              className="bg-green-500 text-white hover:bg-green-600 p-2.5 rounded-xl transition-all shadow-md active:scale-90"
                              title="Aprobar"
                            >
                            ...
                            <button 
                              onClick={() => confirmAction(
                                'Denegar Permiso', 
                                `¿Estás seguro de denegar la salida de ${req.employeeName}?`, 
                                'danger', 
                                () => onReject(req.id)
                              )}
                              className="bg-red-500 text-white hover:bg-red-600 p-2.5 rounded-xl transition-all shadow-md active:scale-90"
                              title="Rechazar"
                            >
                        ...
                        <button 
                          ...
                          className="bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white p-2.5 rounded-xl transition-all"
